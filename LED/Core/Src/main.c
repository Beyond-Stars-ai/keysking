...
int main(void)
{
  ...
  while (1)
  {
    ...
    (state <= 1 || state == 5) ? GPIO_PIN_SET : GPIO_PIN_RESET);
    // 绿灯逻辑: state=1,2,3时亮
    HAL_GPIO_WritePin(LED_GREEN_GPIO_Port, LED_GREEN_Pin, 
    (state >= 1 && state <= 3) ? GPIO_PIN_SET : GPIO_PIN_RESET);
    // 蓝灯逻辑: state=2,3,4时亮
    HAL_GPIO_WritePin(LED_BLUE_GPIO_Port, LED_BLUE_Pin, 
    (state >= 2 && state <= 4) ? GPIO_PIN_SET : GPIO_PIN_RESET);
    HAL_Delay(300);  // 300ms延时

    state = (state >= 5) ? 0 : (state + 1);  // 状态循环0-5
  }
  /* USER CODE END 3 */
}
